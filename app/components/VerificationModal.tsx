...
const VerificationModal: React.FC<VerificationModalProps> = ({ isOpen, onClose, title, verifier, onChainVerification }) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-800 rounded-lg p-3 sm:p-6 w-full max-w-[90vw] sm:max-w-md overflow-y-auto">
        <div className="flex flex-col items-center justify-center mb-2 sm:mb-4">
          <FaCheckCircle className="text-green-400 text-xl sm:text-4xl mb-1 sm:mb-2" />
          <h2 className="text-base sm:text-2xl font-bold text-gray-200 text-center">Article Verified!</h2>
        </div>
        ...
        <div className="mb-2 sm:mb-4">
          <p className="font-semibold text-gray-200 text-xs sm:text-base">Verified by:</p>
          <p className="text-xs sm:text-sm break-all text-gray-300">{verifier}</p>
        </div>
        <div className="mb-3 sm:mb-6">
          ...
          <a href={`https://solana.fm/tx/${onChainVerification}?cluster=devnet-solana`} target="_blank" rel="noopener noreferrer" className="text-xs sm:text-sm break-all text-blue-400 hover:text-blue-300 underline">
            {onChainVerification}
          </a>
        </div>
        <button
          onClick={onClose}
          className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-1.5 sm:py-2 px-4 rounded transition duration-300 text-xs sm:text-base"
        >
          Close
        </button>
      </div>
